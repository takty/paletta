...
export class ChartMunsell extends Chart {
	...
	public getMapZLevel(base: Color, c: Color) {
		const x: number = c.asMunsell()[1];
		const o: number = base.asMunsell()[1];
	...
	public cToXy(c: Color): Pair {
		const t: Triplet = c.asMunsell();

		if (t[0] === -1 || t[2] === -1) {
			return [0.5, 0.5];
		}
		const rad: number = ((t[0] - 30) / 100) * (Math.PI * 2);
		return [
	...
	public xyToC(base: Color, x: number, y: number, dest: Color): void {
		const tb: Triplet = base.asMunsell();

		const xx : number = x - 0.5;
		const yy : number = y - 0.5;
		const rad: number = (yy > 0) ? Math.atan2(yy, xx) : (Math.atan2(-yy, -xx) + Math.PI);
